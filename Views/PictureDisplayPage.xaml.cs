...
public partial class PictureDisplayPage : ContentPage
{
    ...
    public PictureDisplayPage()
    {
        ...
        PictureContainer.GestureRecognizers.Add(backgroundTapGesture);
    }
    ...
    public async void OnSettingClicked(object sender, EventArgs e)
    {
        ...
        {
            isNavigating = true;
            await App.SettingRepo.UpdateBackPage(1, "PictureDisplayPage");
            await Shell.Current.GoToAsync("//SettingPage");
        }
    }

    private async void OnFragmentClicked(object sender, TappedEventArgs e)
    {
        if (!isNavigating)
        {
            // 淡出 post.png 和 fragment
            await Task.WhenAll(
                PostImage.FadeTo(0, 300),
                FragmentImage.FadeTo(0, 300)
            );

            // 淡入 draw_symbol.png
            await DrawSymbolImage.FadeTo(1, 300);
        }
    }
} 